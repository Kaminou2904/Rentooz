...
function Productcard(props) {
  return (
        ...
        <div className='mainProductCard border-brand-skin p-3 pb-2 pt-3'>
            ...
            <Link to={
                window.location.pathname.includes('/buy')? `/buy/${props.cate.replace(/ /g, '-')}/${props.name.replace(/ /g, '-')}-in-pune`
                : `/${props.cate.replace(/ /g, '-')}/${props.name.replace(/ /g, '-')}-on-rent-in-pune`
                } className='nav-link'><img src={props.img} className='img-fluid' alt={props.name} /></Link>
            </div>
            <div className="productCardData">
                <div className="productName">
                    <Link to={
                        window.location.pathname.includes('/buy')? `/buy/${props.cate.replace(/ /g, '-')}/${props.name.replace(/ /g, '-')}-in-pune`
                        : `/${props.cate.replace(/ /g, '-')}/${props.name.replace(/ /g, '-')}-on-rent-in-pune`
                    } className='nav-link'><p className="mb-0 text-brand-blue text-center text-capitalize lh-sm bricolage-extrabold mt-2">{props.name} <br /><span className="text-capitalize bricolage-extrabold">{props.keyfeat}</span></p></Link>
                </div>
