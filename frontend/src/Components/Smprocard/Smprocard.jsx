  ...
  return (
    <div className='mainsmProcard border-brand-skin rounded-brand'>
        ...
        <div className="mainsmProductcardData p-2">
            <Link to={`/${props.category}/${props.name}`} className='nav-link'><p className="mainsmProductcardName bricolage-bold text-capitalize mb-0 text-brand-blue">{props.name}</p></Link>
            <p className="mainsmProductcardCategory mb-0 text-muted text-uppercase">{props.category}</p>
            <div className="mainsmProductcardPrice text-brand-purp lh-sm bricolage-bold">${props.rentprice} <span className='text-muted bricolage-semibold'>/{props.rentperiod}</span></div>
        </div>
    </div>
  );
};
